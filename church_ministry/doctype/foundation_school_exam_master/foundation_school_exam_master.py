...
class FoundationSchoolExamMaster(Document):
	pass


def validate_duplicate(doc,method):
	if doc.get("__islocal"): 
		res=frappe.db.sql("select name from `tabFoundation School Exam Master` where exam_name='%s' and exam_code='%s' and max_score='%s' and min_score='%s'"%(doc.exam_name,doc.exam_code,doc.max_score,doc.min_score))
		if res:
